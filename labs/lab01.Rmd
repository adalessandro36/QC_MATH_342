...
No R is rounding the product to zero since 1 over n goes to zero as n goes to infinity.
...
grid <- seq(from=0,to=1-delta, by=delta)
...
mean(sample(c(0,1), size=500, prob = c(.1, .9), replace=TRUE))
...
x_3_ord <- factor(x_3, levels = c('none', 'infraction', 'misdimeanor', 'felony'))
...
X <- matrix(NA, nrow=length(x_3), ncol=3)
...
Number of individuals with each infraction, misdimeanor, felony. 
...
X[,6] <- sample(c(rep(1, times = 24),rep(0, times=76)))
...
newX$X6 <- ifelse(newX$X6 == 0, 'DOMESTIC', 'FOREIGN')

#alternative answer
newX$X6 <- factor(newX$X6, labels=c('DOMESTIC','FOREIGN'))
...
R <- matrix(sample(c(rep(0, times =1250),rep(1,times=625),rep(2, times =625))), nrow = n, ncol = n)
...
apply(R, 1, FUN = sd, na.rm=TRUE) ; apply(R, 2, FUN = sd, na.rm=TRUE)
...
qnorm(.05,mean=-10,sd=10, lower.tail=TRUE)

#Yes the estimate is extremely close to what is expected theoretically. 
